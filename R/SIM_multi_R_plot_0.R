...
for (rep in replications){
  SIM_multi_path <- paste0("E:/MSPH/EEG methodology/Advanced EEG Code/Code for GitHub/SMGP_BCI_EEG/SIM_multi/replication_", rep, "/R_plots/", paste0("L_", L, "_I_", I))
  SIM_multi_ref_path <- paste0("E:/MSPH/EEG methodology/Advanced EEG Code/Code for GitHub/SMGP_BCI_EEG/SIM_multi_ref/replication_", rep, "/R_plots/", paste0("L_", L, "_I_", I))
  ...
  SIM_test <- SIM_test[1:I,]
  SIM_ref_train <- SIM_ref_train[1:I,]
  SIM_ref_test <- SIM_ref_test[1:I,]
  ...
  b <- ggplot(test, aes(x = Index, y = V1, color = Legend)) +
    ...
    geom_point(shape = 21, size = 3, fill = "white", stroke = 1.5) + 
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.2)) + 
    scale_x_continuous(limits = c(1, I), breaks = seq(1, I + 10, by = 1)) + 
    geom_hline(yintercept = 0.8, linetype = "dashed", color = "black") + 
    geom_hline(yintercept = 0.9, linetype = "dashed", color = "black") + 
    labs(title = "",
         x = "Number of Sequence",
