...
from model_Func import model_single_ref, run_mcmc_single
...
file_path = f'SIM_single_ref/Channel_{channel}/replication_{seed}'
...
test_data = generate_multi_data(Mu_1_true, Mu_0_true, C_t_true, C_e_true, L, I, J, T, E, seed=(seed + 100))
...
X1_df = X1_df[X1_df['Channel'] == channel].iloc[:, :-1]
X1 = X1_df.values

X0_df = train_data[train_data['Y'] == 0].drop(columns=['Y', 'Letter', 'Sequence', 'Flash'])
X0_df = X0_df[X0_df['Channel'] == channel].iloc[:, :-1]
X0 = X0_df.values
...
if __name__ == "__main__":
    samples = run_mcmc_single(model_single_ref, X1, X0, T, N1, N0, s1, s0, gamma, L_1, L_0, Psi_1, Psi_0, V_Psi_1, V_Psi_0, seed)
