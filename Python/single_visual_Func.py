...
def prob_score(odf, X, Beta_1_mean, Beta_0_mean, C_t_mean):
    prob_target = []
    prob_non_target = []
    ...
    for i in range(0, X.shape[0]):
        ...
        s1 = multivariate_normal.logpdf(row_signal, mean=Beta_1_mean, cov=C_t_mean)
        s0 = multivariate_normal.logpdf(row_signal, mean=Beta_0_mean, cov=C_t_mean)
...
def predict(rdf, Beta_1_mean, Beta_0_mean, C_t_mean, T):
    unique_letters = rdf['Letter'].unique()
    unique_sequences = rdf['Sequence'].unique()
    X = rdf.iloc[:, :T].to_numpy()
    odf = rdf.iloc[:, T:]
    odf = prob_score(odf, X, Beta_1_mean, Beta_0_mean, C_t_mean)
